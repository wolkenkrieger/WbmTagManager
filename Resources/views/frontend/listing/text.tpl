...
{block name="frontend_listing_text"}
    ...
    {else}
        <div class="hero-unit category--teaser panel">
            {* Headline *}
            {block name="frontend_listing_text_headline_standard"}
                <h1 class="listing--headline">{s name="HEADLINE_STNARD" namespace="itsw/listing/text"}Unsere aktuellen Angebote zu {$sCategoryContent.name}{if $ItswCar.session.description} für {$ItswCar.session.description}{/if}{/s}</h1>
            {/block}
        </div>
    {/if}
{/block}
